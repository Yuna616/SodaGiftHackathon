...
export default function AdminPage() {
  ...
  function toggleOption(campaignId: string, optionId: string) {
    setSelections((prev) => {
      const next = new Set(prev[campaignId] ?? []);
  ...
  async function resolve(campaignId: string) {
    ...
    setStatus(res.ok ? `발표 완료 · 당첨자 ${data.winnerCount}명` : '처리 실패');
    refresh();
  }
  ...
  return (
    <div className="p-4">
      ...
      <p className="text-xs text-amber-600 bg-amber-50 rounded-lg px-3 py-2 mb-5">
        해커톤 데모용 화면입니다. 인증이 없으니 운영 환경에서는 반드시 접근 제한을 추가하세요.
      </p>
                  ...
                  <input
                    ...
                    checked={
                      c.status === 'resolved'
                        ...
                        : selections[c.id]?.has(opt.id) ?? false
                    }
                    onChange={() => toggleOption(c.id, opt.id)}
                  />
                  {opt.label}
                </label>
              ))}
