...
// 고객사 프로필 화면(배너+아바타+지표+상태 필터 사이드바 + 캠페인 목록) 공용 뷰.
// 스폰서 본인이 보는 대시보드(/dashboard, viewer="owner")와 참가자가 보는 공개
// 프로필(/sponsors/:id, viewer="public")이 이 컴포넌트 하나를 같이 쓴다.
                ...
                <div>
                  ...
                  <dd
                    className={`font-semibold ${
                      stats && (stats.total_failed_claims ?? 0) > 0 ? "text-rose-600" : "text-slate-900"
                    }`}
                  >
                    {stats ? `${stats.total_failed_claims ?? 0}건` : "-"}
                  </dd>
                </div>
              )}
            </dl>
      ...
      <section>
        ...
        {isOwner && (
          <div className="mb-4 flex items-center gap-3 rounded-xl border border-slate-200 bg-white px-4 py-3.5 shadow-sm">
            ...
            {stats && (stats.total_failed_claims ?? 0) > 0 ? (
            ...
            <div className="divide-y divide-slate-100">
              {visibleCampaigns.map((c) => (
                <CampaignRow
                  ...
                  campaign={c}
                  isOwner={isOwner}
                  onDelete={isOwner && c.status === "draft" ? onDelete : undefined}
                />
              ))}
            </div>
          )}
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="flex items-center gap-3">
          {isOwner ? (
            ...
            <a href={`/campaigns/${campaign.id}`} className="text-xs font-medium text-brand-600 hover:underline">
              예측하러 가기 →
            </a>
          )}
        </div>
