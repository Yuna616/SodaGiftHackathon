...
function ClaimDeliveryContent() {
  ...
  useEffect(() => {
    if (!predictionId || !productId) return;
    ...
    fetch('/api/products?country_code=KR')
      .then((r) => r.json())
      .then((data) => setProduct((data.products ?? []).find((p: Product) => p.id === productId) ?? null));
  ...
  async function handleConfirm() {
    ...
    setSubmitting(false);
    if (!res.ok) {
      if (data.error === 'PRODUCT_DISCONTINUED') {
        router.push(`/claim/${predictionId}`);
        return;
      }
